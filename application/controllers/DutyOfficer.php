...
class DutyOfficer extends Admin_Controller 
{
	...
	{   
		...
		if(in_array('usercase',$this->permission))
        {
			...
			$this->data['do']=$this->Globalmodel->getdata_by_field_join_array('duty_officer','do_section','users','u_id',array('do_userid'=>$userid),'do_id', 'asc');
		}
		else
		{
			//$this->data['do']=$this->Globalmodel->getdata_by_field_array('*','duty_officer',array(),'do_id', 'asc');	        
			$this->data['do']=$this->Globalmodel->getdata_by_field_join_array('duty_officer','do_section','users','u_id',array(),'do_id', 'asc');
		}
