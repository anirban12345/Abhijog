...
class Api extends My_Controller 
{
    ...
    public function printHierarchyGraph()
    {
        $this->data['hh']=$this->Globalmodel->getdata_by_field_array('*','hierarchy',array(),'hid', 'asc');

        $div=array();

        foreach($this->data['hh'] as $r)
        {
            $i=0;
            $this->data['hh2']=$this->Globalmodel->getdata_by_field_array('*','hierarchy',array('hparent'=>$r->hid),'hid', 'asc');	                                                      
            foreach($this->data['hh2'] as $x)
            {   
                array_push($div,array('hname'=>$x->hpostname.' - '.$x->hname,'hparent'=>$r->hpostname.' - '.$r->hname));
            } 
        }
        //echo "<pre>";
        //print_r($div);
